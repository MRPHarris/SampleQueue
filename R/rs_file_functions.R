...
#' @param mqblank_subtract TRUE/FALSE average any milliq blanks in the run, and subtract the resulting EEM from all samples/standards/replicates. Defaults to TRUE.
#' @param pblank_subtract TRUE/FALSE to import, dilution process and average any procedural blanks in the run, and subtract the resulting EEM from all samples and replicates. Defaults to TRUE.
...
postprocess_PEM <- function(run_sheet,
                            log_file,
                            mqblank_subtract = TRUE,
                            pblank_subtract = TRUE,
                            neg_to_0 = TRUE,
                            dilution = TRUE){
  ...
  # logs for dilution. Which types will be diluted?
  PEM_sample_log <- log_PEM[which(log_PEM$type == "Sample" | log_PEM$type == "Replicate" | log_PEM$type == "Standard"),]
  ## Dilution  setup for non-blanks, if the option is set to TRUE. Execution comes later.
  if(isTRUE(dilution)){
  ...
  PEM_pblanks <- as.character(log_PEM[which(log_PEM$type == "Procedural Blank"),]$`exported files`)
  ...
  if(isTRUE(any(log$type == "Procedural Blank"))){# & isTRUE(pblank_subtract)){
    ...
    ## Dilute the pblannks with dilution factors listed.
    if(isTRUE(dilution)){
      PEM_pblank_log$dilution <- NA
      # get sample names without extensions
      namespbl <- trim_path(PEM_pblank_log$`exported files`)
      namespbl <- unlist(lapply(str_split(namespbl,"[.]"),"[",1))
      namespbl <- str_remove(namespbl,"PEM")
      # Get samples which have dilution info
      dilfac_spblanks <- run_sheet$Real_Name[which(!is.na(run_sheet$Dilution_Factor))]
      quick_it_list <- vector(mode = "list", length = length(dilfac_spblanks))
      for(q in seq_along(dilfac_spblanks)){
        it_row <- as.numeric(which(namespbl == dilfac_spblanks[q]))
        PEM_pblank_log$dilution[it_row] <- run_sheet$Dilution_Factor[which(!is.na(run_sheet$Dilution_Factor))][q]
      }
      # Get the EEM names from the pblank_eemlist
      eem_names_pre <- unlist(lapply(pblank_eemlist,"[[",'sample'))
      ...
      for(x in seq_along(pblank_dilution_itlist)){
        ...
        which_eem <- as.numeric(which(eem_names_pre == target_name))
        target_eem <- pblank_eemlist[which_eem][[1]]
        dilfac <- as.numeric(target_logrow$dilution)
  ...
  for(t in seq_along(type_itlist)){
    ...
    if(isTRUE(mqblank_subtract)){
      ...
      eems_subtracted <- sample_eemlist # var handover if no blank subtraction is performed.
    }
    ...
    if(type_it == "Sample" | type_it == "Replicate"){
      if(isTRUE(pblank_subtract)){
        if(!isTRUE(any(log$type == "Procedural Blank"))){
          eems_subtracted <- eems_subtracted
        } else {
          eems_subtracted <- eemlist_subtract(eems_minuend = eems_subtracted,
                                              eem_subtrahend = pblank_eem)
          message(paste0("Procedural blank subtraction performed for ",type_it," EEMs"))
        }
      }
    }
    # 3) negative to 0
